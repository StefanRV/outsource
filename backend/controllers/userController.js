const { User, Product, Favorite, Category, FavoriteUser } = require('../models');
...
const userController = {
    ...
    deleteUser: async (req, res) => {
        try {
            ...
            const decodeToken = (token) => {
                try {
                    const secretKey = process.env.JWT_SECRET_KEY || "your-secret-key"; // Secret key
                    const { payload } = jwtVerify(token, Buffer.from(secretKey, 'utf-8'));
                    return payload;
                } catch (error) {
                    console.error('Error while decoding token:', error.message);
    ...
    rechargeBalance: async (req, res) => {
        ...
        const validAmounts = [5, 10, 15, 20, 30, 50].map((val) => parseFloat(val.toFixed(2)));
        ...
        try {
            ...
            res.status(500).json({ message: "Ошибка при пополнении баланса", error: error.message });
        }
    },
    ...
    getUserProfile: async (req, res) => {
      ...
      removeFromFavorites: async (req, res) => {
        try {
          ...
          if (!favorite) {
            return res.status(400).json({ message: 'Продукт не находится в избранном' });
          }
      ...
      getFavorites: async (req, res) => {
        try {
          ...
          res.status(500).json({ message: 'Error with getting favorites', error: error.message });
        }
      },
  ...
  addUserToFavorites: async (req, res) => {
    try {
      ...
      if (!favUser) {
        return res.status(404).json({ message: 'Избранный пользователь не найден' });
      }

      if (userId === favUserId) {
        return res.status(400).json({ message: 'Нельзя добавить себя в избранное' });
      }
      ...
      if (existingFavorite) {
        return res.status(400).json({ message: 'Пользователь уже в избранном' });
      }
  ...
  removeUserFromFavorites: async (req, res) => {
    try {
      ...
      if (!favorite) {
        return res.status(400).json({ message: 'Пользователь не находится в избранном' });
      }
  ...
  getFavoriteUsers: async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Error with getting favorite users', error: error.message });
    }
  },
